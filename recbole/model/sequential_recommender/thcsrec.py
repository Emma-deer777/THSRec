...
class THCSRec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.hglen = config['hyper_len']  # top-k
        self.enable_hg = config['enable_hg']
        self.dataset = config['dataset']
        self.beta = config['beta']  # loss weight
    ...
    def reconstruct_train_data(self, item_seq, last_item):
        """
        ...
        for batch_id in range(batch_size):
            ...
            item_seq[batch_id][n_obj - 1] = last_item[batch_id]
    ...
    def forward(self, item_seq, mask_positions_nums=None, user_id=None):
        ...
        if self.enable_hg:
            ...
            con_loss = self.SSL(hgnn_embs, output)
            # x = x_raw
            # 2, b, l, dim
    ...
    def SSL(self, emb_hgnn, emb_trans):
        ...
        pos = score(emb_hgnn, emb_trans)
        neg1 = score(emb_trans, row_column_shuffle(emb_hgnn))
    ...
    def calculate_loss(self, interaction):
        ...
        last_item = interaction["item_id"]
        masked_item_seq, pos_items, masked_index = self.reconstruct_train_data(item_seq, last_item)

        mask_nums = torch.count_nonzero(pos_items, dim=1)
        seq_output, con_loss = self.forward(masked_item_seq, mask_positions_nums=(masked_index, mask_nums),
                                            user_id=user_id)
        ...
        loss = torch.sum(loss_fct(logits.view(-1, test_item_emb.size(0)), pos_items.view(-1)) * targets) \
               / torch.sum(targets)
        loss = loss + self.beta * con_loss  # 0.00001
        return loss
    ...
    def full_sort_predict(self, interaction):
        ...
        seq_output, con_loss = self.forward(item_seq)
    ...
    def build_Gs_unique(self, seqs, item_sim, group_len):
        ...
        for batch_idx in range(seqs.shape[0]):
            ...
            for i, item in enumerate(seq_list):
                ...
                H[i, ego_idx] = 1.0
                if item in group:
                    H[i, n_unique + group.index(item)] = 1.0
